...
package com.trd.strategies.exp.eurusd_buy_spread.opt;
...
public class ExampleWalker {
    ...
    static class CheckpointWalks {int checkpoint; Walk testWalk, eval1Walk, eval2Walk, eval3Walk, eval4Walk; float checkpointWalkAdjustedProfit;}
    ...
    public static void main(String[] args) throws Exception {
        ...
        var walks = checkpoints.stream()
                ...
                .collect(Collectors.toList());
        var checkpointWalkFutures = new ArrayList<Future<CheckpointWalks>>();
        for (var w : walks) {
            checkpointWalkFutures.add(executorService.submit(() -> {
                var checkpoint = new CheckpointWalks();
                checkpoint.testWalk = runWalk(w, loadSteps("test", w.checkpoint));
                ...
                checkpoint.eval4Walk = runWalk(w, loadSteps("eval4", w.checkpoint));
                checkpoint.checkpointWalkAdjustedProfit = Floats.min(
                        checkpoint.eval1Walk.adjustedProfit * 6,
                        checkpoint.eval2Walk.adjustedProfit * 6,
                        checkpoint.eval3Walk.adjustedProfit * 6,
                        checkpoint.testWalk.adjustedProfit);
                return checkpoint;
            }));
        };
        var checkpointWalks = new ArrayList<CheckpointWalks>();
        for (var f : checkpointWalkFutures) {
            checkpointWalks.add(f.get());
        }
        Collections.sort(checkpointWalks, (w1, w2) -> Floats.compare(
                w1.checkpointWalkAdjustedProfit,
                w2.checkpointWalkAdjustedProfit));
        checkpointWalks.forEach(w ->
                System.out.println("ExampleWalker.main checkpont #" + w.testWalk.checkpoint
                        ...
                        + " \teval4 profit: " + w.eval4Walk.profit + " trades: " + w.eval4Walk.trades.size()
                        + " \tcheckpointWalkAdjustedProfit: " + w.checkpointWalkAdjustedProfit));
        ...
        System.out.println("ExampleWalker.main best eval4 walk:");
        bestWalk.eval4Walk.print();
    ...
    private static Walk findBestWalk(List<WalkStep> trainSteps, List<WalkStep> testSteps, List<WalkStep> evalSteps) throws Exception {
        ...
        bestWalk.maxDrawdown = -1000000;
        bestWalk.winProfit = 1;
        bestWalk.lossProfit = -1000000;
        bestWalk = findBestWalk(walks, bestWalk, false);
        ...
        do {
            ...
            bestWalk = findBestWalk(walks, bestWalk, true);
        ...
        return runWalk(bestWalk.enterBucket, bestWalk.enterThreshold,
                ...
                bestWalk.takeProfitDelta, testSteps, true, false);
    }

    private static Walk findBestWalk(LinkedList<Future<Walk>> walks, Walk bestWalk, boolean consequentLossesLimit) throws Exception {
        while (!walks.isEmpty()) {
            ...
            if (
                    ...
                    // walk.maxDrawdown > bestWalk.maxDrawdown
                    // (walk.adjustedProfit - bestWalk.adjustedProfit) + (walk.maxDrawdown - bestWalk.maxDrawdown) > 0
                            ...
                            // && (walk.maxDrawdown >= -0.005 || !consequentLossesLimit)
            ) {
                bestWalk = walk;
                // bestWalk.print();
            }
    ...
    private static Walk runWalk(Walk walk, List<WalkStep> steps) {
        return runWalk(walk.enterBucket, walk.enterThreshold, walk.exitBucket, walk.exitThreshold,
                walk.stopLossDelta, walk.takeProfitDelta, steps, false, false);
    }
    ...
    static class Walk {
        ...
        public float profit = 0, adjustedProfit, winProfit, lossProfit, enterThreshold, exitThreshold,
                stopLossDelta, takeProfitDelta, maxDrawdown = 0;
        public int wins = 0, losses = 0, stopLosses = 0, takeProfits = 0, enterBucket, exitBucket, consequentLosses;
        ...
        public void calculateStats(boolean optimize) {
            ...
            this.consequentLosses = 0;
            this.maxDrawdown = 0f;
            var currentLosses = 0;
            var currentProfit = 0f;
            var count = 0;
            for (var t : this.trades) {
                count += 1;
                ...
                if (currentLosses > this.consequentLosses) {
                    this.consequentLosses = currentLosses;
                }
            ...
            // this.adjustedProfit = winProfit + lossProfit + maxDrawdown;
            this.adjustedProfit = maxDrawdown;
            if (this.adjustedProfit > -0.004) {
                // if (optimize) {
                //    this.adjustedProfit = calculateAdjustedProfitForOptimization();
                // } else {
                    this.adjustedProfit = winProfit + lossProfit;
                // }
            }
        }

        float calculateAdjustedProfitForOptimization() {
            var trades1 = this.trades.subList(0, trades.size() / 3);
            var trades2 = this.trades.subList(trades.size() / 3, 2 * trades.size() / 3);
            var trades3 = this.trades.subList(2 * trades.size() / 3, trades.size());
            var profit1 = 0f;
            var profit2 = 0f;
            var profit3 = 0f;
            for (var t : trades1) {
            ...
            return Floats.min(profit1, profit2, profit3);
        }
        ...
        public void print() {
            ...
            System.out.println("WalkData.print consequentLosses = " + consequentLosses);
            System.out.println("WalkData.print maxDrawdown = " + maxDrawdown);

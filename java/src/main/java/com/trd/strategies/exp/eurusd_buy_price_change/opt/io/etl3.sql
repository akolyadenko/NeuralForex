...
drop table if exists oanda_candle_bk;
create table oanda_candle_bk as select * from oanda_candle;

update oanda_candle set granularity = lower(granularity);

alter table oanda_candle add time_parsed timestamptz;

update oanda_candle set time = replace(time, '.000000000Z', '');
update oanda_candle set time = replace(time, 'T', '-');
update oanda_candle set time_parsed = to_timestamp(time, 'YYYY-MM-DD-HH24:MI:SS') at time zone 'Etc/UTC';

alter table oanda_candle add epoch_min bigint;
update oanda_candle set epoch_min = extract(epoch from time_parsed) / 60;

alter table oanda_candle add id bigint default nextval('ids');
...
delete from oanda_candle where id not in (select id from ids);

create unique index oanda_candle_pk on oanda_candle(epoch_min, granularity, type);
...
create unique index timeframe_epoch_min_idx on timeframe(epoch_min);
...
-- set split = 'test'
-- where time >= to_timestamp('2022-03-01', 'YYYY-MM-DD')
--    and time < to_timestamp('2022-05-01', 'YYYY-MM-DD');

-- update example
-- set split = 'eval'
-- where time >= to_timestamp('2022-05-01', 'YYYY-MM-DD');
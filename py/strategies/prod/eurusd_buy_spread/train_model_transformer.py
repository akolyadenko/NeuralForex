...
def create_timeframe_subnet(t):
    ...
    for type in ['bid', 'ask']:
        for i in ['high', 'low']:
            ...
            input = layers.Reshape((1, 60))(input)
            ll.append(input)
...
def create_last_timepoint_subnet():
    ...
    for f in data.range_features:
        ...
        inputs[f] = input
        input = tf.nn.dropout(input, 0.2)
        input = layers.Reshape((1, 6))(input)
        ll.append(input)
    input = keras.Input(shape=(), dtype=tf.float32, name='current_bid_rescaled')
    inputs['current_bid_rescaled'] = input
    input = layers.Reshape((1, 1))(input)
    ll.append(input)
    net = layers.concatenate(ll, 2)
    for i in range(2):

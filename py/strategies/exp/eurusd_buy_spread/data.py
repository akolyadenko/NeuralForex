...
for type in ['bid', 'ask']:
    for timeframe in ['m1', 'm5', 'm15', 'm30']:
        for feature in ['open', 'close', 'high', 'low']:
            timeframe_features.append(timeframe + type + feature + '_rescaled')

timeframe_discrete_features = []
for type in ['bid', 'ask']:
    for timeframe in ['m1', 'm5', 'm15', 'm30']:
        for feature in ['open', 'close', 'high', 'low']:
            timeframe_discrete_features.append(timeframe + type + feature + '_rescaled_discrete')

range_features = []
range_discrete_features = []
for type in ['bid', 'ask']:
    for timeframe in ['m1', 'm5', 'm15', 'm30']:
        for feature in ['rangeHigh', 'rangeLow',
                    'rangeHighLinearA', 'rangeHighLinearB', 'rangeLowLinearA', 'rangeLowLinearB']:
            range_features.append(timeframe + type + feature + '_rescaled')
        for feature in ['rangeHigh', 'rangeLow']:
            range_discrete_features.append(timeframe + type + feature + '_rescaled_discrete')
        for feature in [
            'rangeHighProximity5pCounts', 'rangeHighProximity10pCounts', 'rangeHighProximity20pCounts',
                    'rangeLowProximity5pCounts', 'rangeLowProximity10pCounts', 'rangeLowProximity20pCounts',
                        'rangeHighPercentiles', 'rangeLowPercentiles']:
            range_features.append(timeframe + type + feature + '_rescaled')
        for feature in ['rangeHighPercentiles', 'rangeLowPercentiles']:
            range_discrete_features.append(timeframe + type + feature + '_rescaled_discrete')
...
def create_tfrecord_ds(split, batch_size, for_prediction=False):
    ...
    feature_description = {
        ...
        'current_bid_rescaled': tf.io.FixedLenFeature((), tf.float32),
    ...
    for f in timeframe_discrete_features:
        feature_description[f] = tf.io.FixedLenFeature([60], tf.int64)

    for f in range_features:
        feature_description[f] = tf.io.FixedLenFeature([6], tf.float32)
    for f in range_discrete_features:
        feature_description[f] = tf.io.FixedLenFeature([6], tf.int64)
    ...
    def _map_values(x):
        ...
        features['current_bid_rescaled'] = x['current_bid_rescaled']
